...
bin2d_breaks <- function(scale, breaks = NULL, origin = NULL, binwidth = NULL,
  ...
  if (!(is.numeric(binwidth) && length(binwidth) == 1)) cli::cli_abort("{.arg binwidth} must be a {.cls numeric} scalar")

  if (is.null(origin) || identical(origin, NA)) {
    origin <- round_any(range[1], binwidth, floor)
  }
  if (!(is.numeric(origin) && length(origin) == 1)) cli::cli_abort("{.arg origin} must be a {.cls numeric} scalar")
