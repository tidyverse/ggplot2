...
fortify.SpatialPolygonsDataFrame <- function(model, data, region = NULL, ...) {
  ...
  if (is.null(region)) {
    coords <- vec_rbind(!!!lapply(model@polygons,fortify))
...
fortify.SpatialPolygons <- function(model, data, ...) {
  vec_rbind(!!!lapply(model@polygons, fortify))
}
...
fortify.Polygons <- function(model, data, ...) {
  ...
  pieces <- vec_rbind(!!!lapply(seq_along(subpolys), function(i) {
...
fortify.SpatialLinesDataFrame <- function(model, data, ...) {
  vec_rbind(!!!lapply(model@lines, fortify))
}
...
fortify.Lines <- function(model, data, ...) {
  lines <- model@Lines
  pieces <- vec_rbind(!!!lapply(seq_along(lines), function(i) {
    df <- fortify(lines[[i]])
    df$piece <- i
    df
  }))
