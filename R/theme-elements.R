...
element_grob.element_rect <- function(element, x = 0.5, y = 0.5,
  ...
  gp <- gparam(lwd = linewidth, col = colour, fill = fill, lty = linetype)
  element_gp <- gparam(lwd = element$linewidth, col = element$colour,
...
element_grob.element_text <- function(element, label = "", x = NULL, y = NULL,
  ...
  # The gp settings can override element_gp
  gp <- gparam(fontsize = size, col = colour,
    fontfamily = family, fontface = face,
    lineheight = lineheight)
  element_gp <- gparam(fontsize = element$size, col = element$colour,
