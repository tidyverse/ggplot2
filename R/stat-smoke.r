...
#' @export
#' @rdname geom_smoke
stat_smoke <- function(mapping = NULL, data = NULL, geom = "area",
...
#' @usage NULL
#' @export
StatSmoke <- ggproto("StatSmoke", Stat,
  required_aes = "x",
  default_aes = aes(y = ..density.., group = ..adjust.., color = NA),

  compute_group = function(data, scales, adjust = NULL, kernel = "gaussian",
...
compute_smoke <- function(x, w, from, to, bw = "nrd0",
                            adjust = NULL, kernel = "gaussian") {
  n <- length(x)
  if (is.null(w)) {
    w <- rep(1 / n, n)
  }
  ...
  densities <- lapply(adjust, function(a) {
    dens <- stats::density(x, weights = w, bw = bw, adjust = a,
