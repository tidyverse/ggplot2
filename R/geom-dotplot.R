...
geom_dotplot <- function(
  ...
  if (
    ...
    cli::cli_inform(
      "{.code position = \"stack\"} doesn't work properly with {.fn geom_dotplot}. Use {.code stackgroups = TRUE} instead."
    )
  }
...
GeomDotplot <- ggproto(
  ...
  draw_group = function(
    ...
    if (!coord$is_linear()) {
      cli::cli_warn(
        "{.fn geom_dotplot} does not work properly with non-linear coordinates."
      )
    }
    ...
    if (binaxis == "x") {
      ...
      dotdianpc <- dotsize *
        tdata$binwidth[1] /
        (max(panel_params$x.range) - min(panel_params$x.range))
    } else if (binaxis == "y") {
      stackaxis <- "x"
      dotdianpc <- dotsize *
        tdata$binwidth[1] /
        (max(panel_params$y.range) - min(panel_params$y.range))
    }
