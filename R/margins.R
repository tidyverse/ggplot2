  ...
  if (isTRUE(debug)) {
    children <- gList(
      rectGrob(
        ...
        gp = gparam(fill = "cornsilk", col = NA)
      ),
      pointsGrob(x, y, pch = 20, gp = gparam(col = "gold")),
      grob
    )
  } else {
    children <- gList(grob)
...
justify_grobs <- function(grobs, x = NULL, y = NULL, hjust = 0.5, vjust = 0.5,
  ...
  if (isTRUE(debug)) {
    children <- gList(
      rectGrob(gp = gparam(fill = "lightcyan", col = NA)),
      grobs
    )
  }
  else {
  ...
  if (isTRUE(debug)) {
    ...
    grobTree(
      result_grob,
      pointsGrob(x, y, pch = 20, gp = gparam(col = "mediumturquoise"))
    )
  } else {
    result_grob
  }
