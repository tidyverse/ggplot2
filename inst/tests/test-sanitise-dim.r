...
test_that(
  "sanitise_dim returns a NULL for missing inputs, with appropriate warnings",
  {
    n <- NA_integer_
...
test_that(
  "sanitise_dim returns a positive integer or NULL for non-integer inputs, with appropriate warnings",
  {
    n <- 1.5
    expect_identical(suppressWarnings(sanitise_dim(n)), 1L)
