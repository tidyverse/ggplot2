...
- dir:
  ...
  Direction in which panels are laid out. Must be one of the following
  two-letter codes, where the first letter indicates the initial fill
  direction and the pair identifies the starting corner:
  ...
  Use `nrow` or `ncol` to control the shape of the grid; `dir` controls
  the fill order within that shape. The shorthand values `"h"` and `"v"`
  are superseded and retained only for backward compatibility.
