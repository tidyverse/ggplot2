...
A layer is a combination of data, stat and geom with a potential
position adjustment. Usually layers are created using `geom_*` or
`stat_*` calls but it can also be created directly using this function.
...
- check.aes, check.param:
  ...
  If `TRUE`, the default, will check that supplied parameters and
  aesthetics are understood by the `geom` or `stat`. Use `FALSE` to
  suppress the checks.
...
- key_glyph:
  ...
  A legend key drawing function or a string providing the function name
  minus the `draw_key_` prefix. See
...
- layer_class:
  ...
  The type of layer object to be constructed. This is intended for
  ggplot2 internal use only.

## See also

The [plot building chapter](https://ggplot2-book.org/layers) and [geoms
chapter](https://ggplot2-book.org/individual-geoms) of the online
ggplot2 book.
