...
`stat_summary_2d()` is a 2d variation of
[`stat_summary()`](https://ggplot2.tidyverse.org/dev/reference/stat_summary.md).
`stat_summary_hex()` is a hexagonal variation of `stat_summary_2d()`.
The data are divided into bins defined by `x` and `y`, and then the
...
- `z`:
  ...
  After binning, the z values of individual data points are no longer
  available.
