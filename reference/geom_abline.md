...
`geom_abline(intercept = 0, slope = 1)`, then behind the scenes the geom
makes a new data frame containing just the data you've supplied. That
means that the lines will be the same in all facets; if you want them to
vary across facets, construct the data frame yourself and use
aesthetics.
...
These geoms are drawn using
[`geom_line()`](https://ggplot2.tidyverse.org/reference/geom_path.md) so
they support the same aesthetics: `alpha`, `colour`, `linetype` and
...
for a more general approach to adding straight line segments to a plot.
