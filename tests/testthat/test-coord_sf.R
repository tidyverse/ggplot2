...
test_that("axis labels are correct for manual breaks", {
  ...
  plot <- ggplot(sf::st_polygon(list(matrix(
    1e3 * c(1, 2, 3, 1, 1, 3, 2, 1),
    ncol = 2
  )))) +
    geom_sf()
...
test_that("default crs works", {
  ...
  polygon <- sf::st_sfc(
    sf::st_polygon(list(matrix(
      c(-80, -76, -76, -80, -80, 35, 35, 40, 40, 35),
      ncol = 2
    ))),
    crs = 4326 # basic long-lat crs
  )
  ...
  expect_doppelganger(
    ...
    p + geom_point(data = points, aes(x, y)) + coord_sf(default_crs = 4326)
  )
  ...
  expect_doppelganger(
    ...
    p +
      geom_point(data = points_trans, aes(x, y)) +
