...
test_that("plots with empty data and vectors for aesthetics work", {
  ...
  d <- pdata(ggplot(mapping = aes(1:5, 1:5)) + geom_point())
  expect_equal(nrow(d[[1]]), 5)

  d <- pdata(ggplot() + geom_point(aes(1:5, 1:5)))
  expect_equal(nrow(d[[1]]), 5)
})
...
test_that("layers with empty data are silently omitted with facets", {
  ...
  expect_error(ggplot_build(ggplot(df0, aes(mpg, wt)) + geom_point() + facet_wrap(~cyl)))

  # Empty data, facet_grid, throws error
  expect_error(ggplot_build(ggplot(df0, aes(x, y)) + geom_point() + facet_grid(am ~ cyl)))


  # points from mtcars points and points from empty data frame, facet_wrap
  d <- pdata(ggplot(mtcars, aes(mpg, wt)) + geom_point() + geom_point(data = df0) + facet_wrap(~cyl))
  expect_equal(nrow(d[[1]]), nrow(mtcars))
  expect_equal(nrow(d[[2]]), 0)

  # points from mtcars points and points from empty data frame, facet_grid
  d <- pdata(ggplot(mtcars, aes(mpg, wt)) + geom_point() + geom_point(data = df0) + facet_grid(am ~ cyl))
  expect_equal(nrow(d[[1]]), nrow(mtcars))
  expect_equal(nrow(d[[2]]), 0)
})
...
test_that("data is not inherited when when data=data.frame()", {
  ...
  expect_error(ggplot_build(ggplot(mtcars, aes(mpg, wt)) + geom_point() +
