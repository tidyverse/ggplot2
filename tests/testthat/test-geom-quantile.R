...
test_that("geom_quantile matches quantile regression", {
  ...
  pred_rq <- predict(quantreg::rq(y ~ x,
                        tau = quants,
                        data = df),
                     tibble::tibble(
  ...
  expect_equal(ggplot_data[ggplot_data$quantile == 0.25, c("x", "y")],
               pred_rq_test_25)

  pred_rq_test_50 <- pred_rq[, c("x", "Q_50")]
  colnames(pred_rq_test_50) <- c("x","y")

  expect_equal(ggplot_data[ggplot_data$quantile == 0.5, c("x", "y")],
               pred_rq_test_50)

  pred_rq_test_75 <- pred_rq[, c("x", "Q_75")]
  colnames(pred_rq_test_75) <- c("x","y")

  expect_equal(ggplot_data[ggplot_data$quantile == 0.75, c("x", "y")],
               pred_rq_test_75)
})
