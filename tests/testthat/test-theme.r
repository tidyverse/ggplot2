...
test_that("aspect ratio is honored", {
  ...
  expect_copycat("height is 3 times width",
    p + theme(aspect.ratio = 3))
  expect_copycat("height is 3 times width, 2 wrap facets",
    p + facet_wrap(~f) + theme(aspect.ratio = 3)
  )
  expect_copycat("height is 3 times width, 2 column facets",
    p + facet_grid(.~f) + theme(aspect.ratio = 3)
  )
  expect_copycat("height is 3 times width, 2 row facets",
    p + facet_grid(f~.) + theme(aspect.ratio = 3)
  )
  expect_copycat("height is 3 times width, 2x2 facets",
    p + facet_grid(f1~f2) + theme(aspect.ratio = 3)
  )

  expect_copycat("width is 3 times height",
    p + theme(aspect.ratio = 1/3)
  )
  expect_copycat("width is 3 times height, 2 wrap facets",
    p + facet_wrap(~f) + theme(aspect.ratio = 1/3)
  )
  expect_copycat("width is 3 times height, 2 column facets",
    p + facet_grid(.~f) + theme(aspect.ratio = 1/3)
  )
  expect_copycat("width is 3 times height, 2 row facets",
    p + facet_grid(f~.) + theme(aspect.ratio = 1/3)
  )
  expect_copycat("width is 3 times height, 2x2 facets",
...
test_that("themes are drawn in the right style", {
  ...
  expect_copycat("theme_bw() plus blue text",
    p + theme_bw() + theme(text = element_text(colour = 'blue')),
  )

  t <- theme_bw() + theme(text = element_text(colour = 'blue'))
  expect_copycat("add saved theme object with theme_bw() plus blue text",
    p + t
  )
  expect_copycat("blue text plus theme_bw() - result is black text",
    p + theme(text = element_text(colour = 'blue')) + theme_bw()
  )

  t <- theme(text = element_text(colour = 'blue')) + theme_bw()
  expect_copycat("add blue saved theme object and theme_bw() - expect black text",
    p + t
  )
  expect_copycat("add blue and italic in single element object",
    p + theme(text = element_text(colour = 'blue', face = 'italic'))
  )
  expect_copycat("add blue and italic in separate element objects",
    p +
  ...
  # Inheritance tests
  expect_copycat('add theme_bw(base_size=24, base_family="serif")',
    p + theme_bw(base_size = 24, base_family = "serif") + labs(title = "Title text here")
  )
  expect_copycat("axis title text is blue, compounded relative sizing",
    p + theme_bw() +
  ...
  expect_copycat("theme_bw + larger relative size for axis.title.y",
    p + theme_bw() + theme(axis.title.y = element_text(size = rel(2)))
  )

  expect_copycat("theme_bw %+replace% larger relative size for axis.title.y",
  ...
  expect_copycat("text is element_blank - result is no text",
    p + theme_bw() + theme(text = element_blank())
  )

  # Testing specific elements
  expect_copycat("many blank items, and light blue plot background",
    p + theme(
