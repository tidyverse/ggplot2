test_that("geom_ribbon() checks the aesthetics", {
  ...
  p <- ggplot(huron) +
    geom_ribbon(
      aes(year, ymin = level - 5, ymax = level + 5),
      orientation = "y"
    )
  expect_snapshot_error(ggplotGrob(p))
  p <- ggplot(huron) +
    geom_ribbon(
      aes(y = year, xmin = level - 5, xmax = level + 5),
      orientation = "x"
    )
  expect_snapshot_error(ggplotGrob(p))
  p <- ggplot(huron) +
  ...
  expect_snapshot_error(geom_ribbon(
    aes(year, ymin = level - 5, ymax = level + 5),
    outline.type = "test"
  ))
})
...
test_that("outline.type option works", {
  ...
  g_area_default <- get_layer_grob(
    ggplot(df, aes(x, y)) + geom_area(stat = "identity")
  )[[1]]
  ...
  expect_equal(
    g_ribbon_default$children[[1]]$children[[2]]$id,
    rep(c(1L, 2L), each = 4)
  )
