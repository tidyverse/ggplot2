...
test_that("minor breaks draws correctly", {
  ...
  expect_copycat("manual minor breaks", p)
  expect_copycat("manual minor breaks with coord_polar",
    p + coord_polar()
  )
  ...
  expect_copycat("months and weeks breaks", p)
  expect_copycat("months and weeks breaks: coord polar", p + coord_polar())
  expect_copycat("default breaks",
    ggplot(NULL, aes(letters[1:3], 1:3)) + geom_point()
  )
  expect_copycat("scale_x_continuous(trans = log2_trans()) + scale_y_log10",
    qplot(1:1e4, 1:1e4) + scale_x_continuous(trans = log2_trans()) + scale_y_log10()
  )
  expect_copycat("x and y transformations",
