...
test_that("geom_dotplot draws correctly", {
  ...
  expect_copycat("bin y, three x groups, stack centerwhole",
    ggplot(dat2, aes(x, y)) + geom_dotplot(binwidth = .25, binaxis = "y", stackdir = "centerwhole")
  )
  expect_copycat("bin y, three x groups, bins aligned across groups",
    ggplot(dat2, aes(x, y)) + geom_dotplot(binwidth = .25, binaxis = "y", stackdir = "center", binpositions = "all")
  )
  expect_copycat("bin y, three x groups, bins aligned, coord_flip",
    ggplot(dat2, aes(x, y)) + geom_dotplot(binwidth = .25, binaxis = "y", stackdir = "center", binpositions = "all") +
      coord_flip()
  )
  expect_copycat("bin y, dodged",
  ...
  expect_copycat("bin y, dodged, coord_flip",
    ggplot(dat2, aes("foo", y, fill = x)) + scale_y_continuous(breaks = seq(-4, 4, .4)) +
      geom_dotplot(binwidth = .25, position = "dodge", binaxis = "y", stackdir = "center") +
      coord_flip()
  )
  expect_copycat("bin y, three x groups, fill and dodge",
  ...
  expect_copycat("bin y, dodging, 3 stackgroups, histodot (currently broken)",
  ...
  expect_copycat("facets, 3 groups, histodot, stackgroups",
  ...
  if (!interactive()) {
    expect_warning(expect_copycat("2 NA values, dot-density binning, binwidth = .4",
      ggplot(dat2, aes(x)) + geom_dotplot(binwidth = .4)
    ))
    expect_warning(expect_copycat("2 NA values, bin along y, stack center",
