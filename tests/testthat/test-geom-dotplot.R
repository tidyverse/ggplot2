...
set.seed(111)
dat <- data_frame(
  x = rep(LETTERS[1:2], 15),
  y = rnorm(30),
  g = rep(LETTERS[3:5], 10)
)
...
test_that("NA's result in warning from stat_bindot", {
  ...
  expect_snapshot_warning(ggplot_build(
    ggplot(dat, aes(x)) + geom_dotplot(binwidth = 0.2)
  ))
})
...
test_that("when binning on y-axis, limits depend on the panel", {
  ...
  equal_limits1 <- (b1@layout$panel_params[[1]]$y.range ==
    b1@layout$panel_params[[2]]$y.range)
  equal_limits2 <- (b2@layout$panel_params[[1]]$y.range ==
    b2@layout$panel_params[[2]]$y.range)
...
test_that("geom_dotplot draws correctly", {
  ...
  expect_doppelganger(
    ...
    ggplot(dat, aes(x)) +
      geom_dotplot(binwidth = 0.4, stackratio = 0.5, fill = "white")
  )
  expect_doppelganger(
    "larger dots: dotsize=1.5, fill=white",
    ggplot(dat, aes(x)) +
      geom_dotplot(binwidth = 0.4, dotsize = 1.4, fill = "white")
  )
  ...
  expect_doppelganger(
    ...
    ggplot(dat, aes(x)) +
      geom_dotplot(binwidth = 0.4, stackdir = "up") +
      coord_flip()
  )
  expect_doppelganger(
    "stack down with coord_flip",
    ggplot(dat, aes(x)) +
      geom_dotplot(binwidth = 0.4, stackdir = "down") +
      coord_flip()
  )
  expect_doppelganger(
    "stack center with coord_flip",
    ggplot(dat, aes(x)) +
      geom_dotplot(binwidth = 0.4, stackdir = "center") +
      coord_flip()
  )
  expect_doppelganger(
    "stack centerwhole with coord_flip",
    ggplot(dat, aes(x)) +
      geom_dotplot(binwidth = 0.4, stackdir = "centerwhole") +
      coord_flip()
  )
  ...
  expect_doppelganger(
    ...
    ggplot(dat, aes(x, fill = g)) +
      geom_dotplot(binwidth = 0.4, alpha = 0.4, binpositions = "all")
  )
  ...
  expect_doppelganger(
    ...
    ggplot(dat, aes(0, x)) +
      geom_dotplot(binwidth = 0.4, binaxis = "y", stackdir = "center")
  )
  expect_doppelganger(
    "bin along y, stack centerwhole",
    ggplot(dat, aes(0, x)) +
      geom_dotplot(binwidth = 0.4, binaxis = "y", stackdir = "centerwhole")
  )
  expect_doppelganger(
  ...
  dat2 <- data_frame(
    x = rep(factor(LETTERS[1:3]), 30),
    y = rnorm(90),
    g = rep(factor(LETTERS[1:2]), 45)
  )

  expect_doppelganger(
    "bin x, three y groups, stack centerwhole",
    ggplot(dat2, aes(y, x)) +
      geom_dotplot(binwidth = 0.25, binaxis = "x", stackdir = "centerwhole")
  )
  expect_doppelganger(
    "bin y, three x groups, stack centerwhole",
    ggplot(dat2, aes(x, y)) +
      geom_dotplot(binwidth = 0.25, binaxis = "y", stackdir = "centerwhole")
  )
  expect_doppelganger(
  ...
  expect_doppelganger(
    "bin y, dodged",
    ggplot(dat2, aes("foo", y, fill = x)) +
      scale_y_continuous(breaks = seq(-4, 4, 0.4)) +
      geom_dotplot(
        binwidth = 0.25,
        position = "dodge",
        binaxis = "y",
  ...
  expect_doppelganger(
    ...
    ggplot(dat2, aes(as.numeric(x), y, group = x)) +
      geom_dotplot(binwidth = 0.2, binaxis = "y", stackdir = "center")
  )
  expect_doppelganger(
  ...
  expect_doppelganger(
    ...
    ggplot(
      ...
      continuous_scale("stroke", palette = function(x) {
        scales::rescale(x, to = c(1, 6))
      }) +
  ...
  # Stacking groups
  expect_doppelganger(
    "3 stackgroups, dot-density with aligned bins",
