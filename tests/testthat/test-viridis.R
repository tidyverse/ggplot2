...
test_that("viridis scale changes point color", {
  ...
  expect_false(get_layer_data(p1)$colour == get_layer_data(p2)$colour)
  expect_equal(get_layer_data(p2)$colour, "#440154FF")
})

test_that("viridis scale is used by default for ordered factors", {
  p <- ggplot(df, aes(x, y, colour = tier)) + geom_point()

  expect_equal(get_layer_data(p)$colour, "#440154FF")
})
