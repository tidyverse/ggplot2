...
test_that("maps draws correctly", {
  ...
  expect_copycat("no projection",
    pworld
  )
  expect_copycat("mercator projection",
    pworld + coord_map(projection = "mercator")
  )
  expect_copycat("ortho projection, default orientation (centered on north pole)",
    pworld + coord_map(projection = "ortho") +
      scale_y_continuous(breaks = (-2:2) * 30) +
      scale_x_continuous(breaks = (-4:4) * 45)
  )
  expect_copycat("ortho projection, custom orientation (centered on New York)",
    pworld + coord_map(projection = "ortho", orientation = c(41, -74 ,0)) +
      scale_y_continuous(breaks = (-2:2) * 30) +
      scale_x_continuous(breaks = (-4:4) * 45)
  )
  # Need to set limits here so left-most longitude line shows up
  expect_copycat("aitoff projection, default orientation",
  ...
  expect_copycat("aitoff projection, custom orientation (centered on date line)",
  ...
  expect_copycat("USA map, mercator projection",
