...
test_that("grid: single row and single col equivalent", {
  row <- facet_grid(a~.)$layout(list(a))
  col <- facet_grid(.~a)$layout(list(a))

  expect_equal(row$ROW, 1:2)
  expect_equal(row$ROW, col$COL)
  expect_equal(row[c("PANEL", "a")], col[c("PANEL", "a")])

  row <- facet_grid(a~.)$layout(list(a, b))
  col <- facet_grid(.~a)$layout(list(a, b))
...
test_that("wrap: drop = FALSE preserves unused levels", {
  wrap_a <- facet_wrap(~a, drop = FALSE)$layout(list(a2))
  expect_equal(nrow(wrap_a), 4)
  expect_equal(as.character(wrap_a$a), as.character(1:4))

  wrap_b <- facet_wrap(~b, drop = FALSE)$layout(list(a2))
...
test_that("missing values get a panel", {
  wrap_a <- facet_wrap(~a)$layout(list(a3))
  wrap_b <- facet_wrap(~b)$layout(list(a3))
  wrap_c <- facet_wrap(~c)$layout(list(a3))
  grid_a <- facet_grid(a~.)$layout(list(a3))
  grid_b <- facet_grid(b~.)$layout(list(a3))
  grid_c <- facet_grid(c~.)$layout(list(a3))
