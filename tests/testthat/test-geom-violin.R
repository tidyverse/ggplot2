test_that("range is expanded", {
  ...
  expect_equal(
    get_panel_scales(p, 1)$y$dimension(),
    c(0 - expand_a, 1 + expand_a)
  )
  expect_equal(
    get_panel_scales(p, 2)$y$dimension(),
    c(0 - expand_b, 2 + expand_b)
  )
})
...
test_that("create_quantile_segment_frame functions for 3 quantiles", {
  ...
  expect_equal(
    create_quantile_segment_frame(density.data, qs)$y,
    rep(qs, each = 2)
  )
})
...
test_that("geom_violin draws correctly", {
  ...
  expect_doppelganger("basic", ggplot(dat, aes(x = x, y = y)) + geom_violin())
  expect_doppelganger(
  ...
  expect_doppelganger(
    "continuous x axis, many groups (center should be at 2.0)",
