...
render <- function(panels, layout, x_scales, y_scales, ranges, coord, data,
  ...
  if (params$horizontal) {
    ...
    panel_margin <- if (is.null(theme$panel.margin.x)) {
      theme$panel.margin
    } else {
      theme$panel.margin.x
    }
    panel_table <- gtable::gtable_add_col_space(panel_table, panel_margin)
  } else {
    panels <- matrix(panels, ncol = 1)
    panel_table <- gtable::gtable_matrix("layout", panels, 
      widths = unit(1, "null"), heights = unit(c(1, 1), "null"), clip = "on")
    panel_margin <- if (is.null(theme$panel.margin.y)) {
      theme$panel.margin
    } else {
      theme$panel.margin.y
    }
    panel_table <- gtable::gtable_add_row_space(panel_table, panel_margin)
  }
  # Name panel grobs so they can be found later
  panel_table$layout$name <- paste0("panel-", c(1, 2))
  
  # Construct the axes
  axes <- Facet$render_axes(ranges[1], ranges[1], coord, theme, 
    transpose = TRUE)

  # Add axes around each panel
  panel_pos_h <- Facet$panel_cols(panel_table)$l
  panel_pos_v <- Facet$panel_rows(panel_table)$t
...
FacetTrans <- ggproto("FacetTrans", Facet,
  ...
  draw_panels = function(panels, layout, x_scales, y_scales, ranges, coord,
    ...
    # Construct the axes
    axes <- Facet$render_axes(ranges[1], ranges, coord, theme, 
      transpose = TRUE)
