...
Next we write a layer function. In an ideal world, I would have originally called these things layers, so you'd use `layer_chull()` instead of `stat_chull()`, but unfortunately changing things now would require a huge amount of effort.
...
Now we can try out our new Stat:
...
This Stat is very inflexible because it has no parameters. We might want to allow the user to control the model formula and the number of points used to generate the grid. To do so, we add arguments to the `compute_group()` method and our wrapper function:
...
stat_density2 <- function(mapping = NULL, data = NULL, geom = "line",
                    position = "identity", show.legend = NA, 
                    inherit.aes = TRUE, bandwidth = NULL,
                    ...) {
  layer(
    stat = StatDensity2, data = data, mapping = mapping, geom = geom, 
    position = position, show.legend = show.legend, inherit.aes = inherit.aes,
...
ggplot(mpg, aes(displ, colour = drv)) + 
  stat_density2(bandwidth = 1)
```

In this scenario, I recommend using `NULL` as a default value, and when picking important parameters you should `message()` the user with the value you chose. (And when printing a floating point parameter, using `signif()` to show only a few significant figures).

### Variable names and default aesthetics

This stat also illustrates another important point. If we want to make this stat usable with other geoms, we should return a variable called `density` instead of `y`. Then we can set up the `default_aes` to automatically map `density` to `y`, which allows the user to override it to use with different geoms:
...
ggplot(mpg, aes(displ, drv, colour = ..density..)) + 
  stat_density2(bandwidth = 1, geom = "point")
```
...
ggplot(mpg, aes(displ, fill = drv)) + 
  stat_density2(bandwidth = 1, geom = "area", position = "stack")
```
...
ggplot(mpg, aes(displ, drv, fill = ..density..)) + 
  stat_density2(bandwidth = 1, geom = "raster")
```

(A better use would be to display with `geom_tile()` or `geom_raster()`, but this requires the x's for each group to line up.)

### Exercises

1.  Extend `stat_chull` to compute the alphahull, as computed by the
    [alphahull](https://cran.r-project.org/package=alphahull) package. Your
    new stat should take an `alpha` argument.

1.  Modify the final version of `StatDensity2` to allow the user to also 
...
geom_chull <- function(mapping = NULL, data = NULL, 
                       ...
                       inherit.aes = TRUE, ...) {
  layer(
    stat = StatChull, geom = GeomPolygon, data = data, mapping = mapping,
    position = position, show.legend = show.legend, inherit.aes = inherit.aes,
    params = list(...)
  )
}
